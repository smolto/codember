...
// SOLUTION: perro2gato3dios2
const splitInput = input.toLocaleLowerCase().trim().split(' ');

const findRepeat = splitInput.reduce((prev, current) => {
  const index = prev.findIndex(({key, value}) => key === current)
  if(index === -1) {
    prev.push({key: current, value: 1})
    return prev
  }
  const auxPrev = [...prev]
  auxPrev[index] = {key: current, value: auxPrev[index].value + 1}
  return auxPrev;
}, [])

const result = findRepeat.reduce((prev, current) => {
  return `${prev}${current.key}${current.value}`
}, '');

console.timeEnd('challenge01');
console.log(`subimt ${result}`)
