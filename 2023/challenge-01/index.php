<?php 
  function findAndUpdate(&$list, $key) {
    $isFound = false;
    ...
    if(!$isFound) {
      $list[] = array('key' => $key, 'value' => 1);
    }
  }

  function joinKeyValue(&$list) {
    $result = "";
    foreach ($list as $item) {
  ...
  foreach ($splitInput as $item) {
    findAndUpdate($findRepeat, $item);
  }

  $result = joinKeyValue($findRepeat)."\n";
  $end_time = microtime(true); 
  
  // Calculate script execution time 
  $execution_time = round(($end_time - $start_time) * 1000, 3);

  echo "challenge01: ".$execution_time."ms\n"; 
  echo "submit " . $result . "";
?>